...
<div class="rpanel-title"> {{ ucwords($application->full_name) }}
    <a style="cursor-pointer" onclick="window.location.reload()"><i style="font-size:30px;"
                                                                    class="fa fa-times-circle fa-lg right-side-toggle"
    ...
    <div id="tabs">
        ...
        <div id="tabs-1">
            <div class="row font-12">
                <div class="col-4">
                    ...
                    @else
                        <img src="{{ asset('user-uploads/candidate-photos/'.$application->photo) }}" class=" img-fluid"
                             width="150">
                    @endif
                ...
                <div class="col-8 right-panel-box">
                    ...
                    <div class="d-flex ml-1">
                        @if($user->can('add_schedule') && is_null($application->schedule))
                            <a class="mr-1"
                               style="border: 0.1px solid; border-radius: 5px; padding:0px 5px;right: 16px;color:#007bff"
                               onclick="createSchedule('{{$application->id}}')"
                               href="javascript:;">@lang('modules.interviewSchedule.scheduleInterview')</a>
                        @endif
                        <a style="border: 0.1px solid; border-radius: 5px; padding:0px 5px;right: 16px;color:#007bff"
                           href="#" class="share-application" data-application-id="{{ $application->id }}"
                           data-toggle="modal" data-target="#emailModal"
                        >Share Candidate <i class="fa fa-share"></i></a>
                    </div>
                </div>
                <hr>
        ...
        <div id="tabs-3">
            ...
            @forelse($answers as $answer)
                <?php $sn = $sn + 1; ?>
                <div class="col-sm-12">
                    <p class="pull-left" style="display:inline-block"><b>Q{{$sn}}
                            .</b>&nbsp;{{$answer->question->question}}&nbsp;&nbsp; </p>
                    <p class="stars stars-example-fontawesome">
                        <select class="question_rating_class" style="display:inline-block" id="{{$answer->id}}"
                                name="question_rating" autocomplete="off">
                            <option value=""></option>
                            @if($answer->score == 1)
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <input type="hidden" value="{{auth()->user()->name }}" name="recruiterName">
                        </div>
                        <div class="form-group">
    ...
    <div class="ui-tabs ui-widget ui-widget-content ui-corner-all mt-3">
        <div class="comment-sections">
            ...
            <div class="post-comment">
                <textarea placeholder="Post a comment" class="form-control" id="postComment" rows="3"></textarea>
                <button id="postCommentButton">Comment</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    $('#postCommentButton').click(function(event){
        ...
        $.easyAjax({
                url: '/comment',
                type: "POST",
                data: {"user_id": '{{$user->id}}',"job_application_id": '{{$application->id}}',"comment" : comment.val(), "_token": '{{csrf_token()}}' },
                success: function (response) {
