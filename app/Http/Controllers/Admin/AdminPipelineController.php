...
use App\JobApplicationTeam;
use App\ScheduleComments;

class AdminPipelineController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        return view('admin.pipeline.board', $this->data);
    }

    public function reservecandidate(Request $request){
        $appTeam = JobApplication::find($request->application_id);
        $appTeam->team_member_id = $this->user->id;

        $appTeam ->save();

        return response()->json(['name'=> $this->user->name, 'initial'=>substr($this->user->name, 0, 1)  ]);
    }


     public function pipeline($jobid)
     {
        abort_if(! $this->user->can('view_job_applications'), 403);
        ...
        $this->applied_job = JobApplication::where('job_id',$jobid)->where('status_id',"1")->count();
        $this->hired = JobApplication::where('job_id',$jobid)->where('status_id',"4")->count();
        $this->interview = JobApplication::where('job_id',$jobid)->where('status_id',"3")->count();
        $this->rejected = JobApplication::where('job_id',$jobid)->where('status_id',"5")->count();
        $this->phonescreen = JobApplication::where('job_id',$jobid)->where('status_id',"2")->count();
        ...
        $this->app_teams = User::where('company_id',$this->user->company_id)->get();
    ...
    public function create(){
        ...
        return view('admin.pipeline.create', $this->data);
    }
    ...
    public function jobQuestion($jobID){
        ...
        $view = view('admin.pipeline.job-question', $this->data)->render();
    ...
    public function edit($id){
        ...
        return view('admin.pipeline.edit', $this->data);
    }
    ...
    public function createSchedule(Request $request, $id){
        ...
        return view('admin.pipeline.interview-create', $this->data)->render();
    }
    ...
    public function scheduleInterview(Request $request){
        ...
        $interviewSchedule->job_application_id = $request->candidate_id;
        ...
        if($request->comments){
            ...
            $scheduleComments->comment = $request->comments;
        ...
        if(!empty($request->team_members)){
            ...
            foreach($request->team_members as $employee)
            {
        ...
        return Notification::send($jobApplication, new CandidateScheduleInterview($jobApplication, $interviewSchedule));
    }
    ...
    public function store(StoreJobApplication $request){
        ...
        return Reply::redirect(route('admin.pipeline.index'), __('menu.jobApplications').' '.__('messages.createdSuccessfully'));
    }
    ...
    public function update(UpdateJobApplication $request, $id){
        ...
        return Reply::redirect(route('admin.pipeline.index'), __('menu.jobApplications').' '.__('messages.updatedSuccessfully'));
    }
    ...
    public function show($id){
        ...
        $view = view('admin.pipeline.show', $this->data)->render();
    ...
    public function table()
    {
        ...
        $this->jobs = Job::all();
        return view('admin.pipeline.index', $this->data);
    }
    ...
    public function export($status, $location, $startDate, $endDate, $jobs) {
        ...
        Excel::create('pipeline', function($excel) use ($exportArray) {
