...
class AdminJobApplicationController extends AdminBaseController
{
    ...
    public function data(Request $request){
        ...
        $jobApplications = JobApplication::select('job_applications.id','job_applications.score_percent','jobs.is_remote','job_applications.full_name','job_applications.resume', 'jobs.title', 'jobs.job_location', 'application_status.status')
        ...
        return DataTables::of($jobApplications)
            ...
            ->editColumn('score', function ($row) {
                return number_format((float)$row->score_percent, 0, '.', '').'%';
            })
    ...
    public function createSchedule(Request $request, $id){
        ...
        $this->timezone = CompanySetting::where('id',$this->user->company_id)->first();
    ...
    public function storeSchedule(StoreRequest $request){
        ...
        $timeZone = CompanySetting::where('id', $this->user->company_id)->first()->timezone;

        // mail to candidate for inform interview schedule
        Notification::send($jobApplication, new CandidateScheduleInterview($jobApplication, $interviewSchedule, $timeZone,$comment));
    ...
    public function scoreSave(Request $request, $id)
    {
        abort_if(! $this->user->can('edit_job_applications'), 403);

        $application_id = explode("-",$id)[0];
        $question_id = explode("-",$id)[1];



        $jsa = JobApplicationAnswer::findOrFail($question_id);
        $jsa->score = $request->rating;
        $jsa->save();
        ...
        $msql = "select sum(score) Score from job_application_answers where job_application_id=".$application_id." group by job_application_id";
        $totalScore = DB::select($msql);

        $application = JobApplication::findOrFail($application_id);
        $application->score = $totalScore[0]->Score;
        $application->score_percent = (intval($totalScore[0]->Score)*100)/$totalMarks;
        $application->save();
